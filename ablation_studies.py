#!/usr/bin/env python3
"""
Ablation Studies for PrivacyMAS
...
class AblationStudy:
    """Conducts ablation studies on PrivacyMAS components"""
    ...
    def _initialize_dataset(self):
        """Initialize dataset for ablation studies"""
        ...
        if self.config.domain == "medical":
            ...
            self.dataset_manager.process_cases_for_coordination(num_cases=100)
            self.domain_config = med_config
        else:
            logger.info("Initializing finance dataset for ablation...")
            fin_config = FinanceConfig(max_samples=200, use_llm=False)
    ...
    def _ablate_hierarchical_coordination(self) -> pd.DataFrame:
        ...
        for run in range(self.config.num_runs):
            ...
            cluster_sizes = [
                ...
                10,  # Small clusters
                5,   # Medium clusters
                2    # Small clusters (binary tree-like)
            ]
    ...
    def _ablate_privacy_mechanisms(self) -> pd.DataFrame:
        """Test different privacy mechanisms"""
            ...
            for run in range(self.config.num_runs):
                ...
                for episode in range(self.config.num_episodes):
                    ...
                    if mech_config['mechanism'] is None:
                        ...
                        # Override privacy manager temporarily
                        env.privacy_manager.epsilon = float('inf')
                    else:
                        # Use specified mechanism
                        original_apply = env.privacy_manager.apply_noise
                        
                        def custom_apply(data):
                            return env.privacy_manager.apply_noise(
                                data, mechanism=mech_config['mechanism']
                            )
                    ...
                    utilities.append(coord_result.utility_score)
                    privacy_losses.append(coord_result.privacy_loss if mech_config['mechanism'] else 1.0)
    ...
    def _ablate_agent_sensitivity(self) -> pd.DataFrame:
        ...
        for n_agents in agent_counts:
            ...
            for run in range(self.config.num_runs):
                ...
                for episode in range(min(50, self.config.num_episodes)):  # Fewer episodes for large agent counts
                    state = env.reset()
                    coord_result, _ = env.step(state['observations'])
    ...
    def _create_environment(self, num_agents: Optional[int] = None, 
        ...
        """Create environment for ablation testing"""
        
        num_agents = num_agents or self.config.num_agents
        cluster_size = cluster_size or min(10, num_agents // 2)
    ...
    def _generate_ablation_plots(self):
        ...
        if 'adaptive_privacy' in self.results:
            ...
            x = range(len(grouped))
            ax.bar(x, grouped['avg_utility'], alpha=0.7)
            ax.set_xticks(x)
        ...
        if 'environmental_learning' in self.results:
            ...
            ax.bar([i + width/2 for i in x], grouped['avg_adaptation'] * 5,  # Scale for visibility
                  width, label='Adaptation (×5)', alpha=0.7)
        ...
        if 'attack_detection' in self.results:
            ...
            line3 = ax2.plot(grouped['threshold'], grouped['avg_privacy_loss'], 
                           'r-', marker='^', label='Privacy Loss')
            
            ax.set_xlabel('Detection Threshold')
            ax.set_ylabel('Detection Rate / Utility')
            ax2.set_ylabel('Privacy Loss')
        ...
        if 'agent_sensitivity' in self.results:
            ...
            ax2.errorbar(grouped['num_agents'],
                        ...
                        marker='s', label='Time', linewidth=2, 
                        capsize=5, color='orange')
            
            ax.set_xlabel('Number of Agents')
            ax.set_ylabel('Utility Score')
            ax2.set_ylabel('Coordination Time (s)')
